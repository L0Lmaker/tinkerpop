...
* Fixed three data races in `gremlin-go` surfaced by `go test -race` when a connection is closed by the server mid-flight: unsynchronized writes/reads of `channelResultSet.err` between the read loop's error path and a caller blocked in `One()`/`All()`/`GetError()`, and unsynchronized accesses of `connection.state` between the read loop's `errorCallback`, the caller's `Client.Close` path, and `loadBalancingPool.getLeastUsedConnection`.
